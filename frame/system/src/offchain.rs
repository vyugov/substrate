...
impl<Public, Signature, TAnyAppPublic> Signer<Public, Signature> for TAnyAppPublic
where
	TAnyAppPublic: RuntimeAppPublic + AppPublic + From<<TAnyAppPublic as AppPublic>::Generic>,
	<TAnyAppPublic as RuntimeAppPublic>::Signature: AppSignature,
	Signature: From<<<TAnyAppPublic as RuntimeAppPublic>::Signature as AppSignature>::Generic>,
	Public: TryInto<<TAnyAppPublic as AppPublic>::Generic>,
{
	fn sign<Payload: Encode>(public: Public, raw_payload: &Payload) -> Option<Signature> {
		raw_payload.using_encoded(|payload| {
			...
			TAnyAppPublic::from(public)
				.sign(&payload)
				.map(
					<<TAnyAppPublic as RuntimeAppPublic>::Signature as AppSignature>::Generic::from,
				)
				.map(Signature::from)
		})
	}
...
pub type PublicOf<T, Call, X> =
	<<X as SignAndSubmitTransaction<T, Call>>::CreateTransaction as CreateTransaction<
		T,
		<X as SignAndSubmitTransaction<T, Call>>::Extrinsic,
	>>::Public;
...
impl<T, E, S, C, Call> SignAndSubmitTransaction<T, Call> for TransactionSubmitter<S, C, E>
where
	T: crate::Trait,
...
impl<T, C, E, S, Call> SubmitSignedTransaction<T, Call> for TransactionSubmitter<S, C, E>
where
	T: crate::Trait,
	C: CreateTransaction<T, E>,
	E: ExtrinsicT<Call = Call> + codec::Encode,
	...
	fn find_local_keys(
		...
		if let Some(accounts) = accounts {
			...
			accounts
				.into_iter()
				.filter_map(|acc| {
					let idx = local_accounts_and_keys
						.binary_search_by(|a| a.0.cmp(&acc))
						.ok()?;
