...
use threshold_crypto::{
    Ciphertext, DecryptionShare, PublicKey, PublicKeySet, PublicKeyShare, SecretKeySet, serde_impl::SerdeSecret,
    ...
    fn new(n_actors: usize, threshold: usize) -> Self {
        ...
        let actors = (0..n_actors)
            .map(|id| {
                ...
                println!("Secret : {}",HexFmt(bincode::serialize(&SerdeSecret(&sk)).expect("").to_vec()));
                println!("Public: {}",HexFmt(pk.to_bytes().to_vec()));
                 
                let sk_share = sk_set.secret_key_share(id);
                println!("Secret share: {}",HexFmt(bincode::serialize(&SerdeSecret(&sk_share)).expect("").to_vec()));
                let pk_share = pk_set.public_key_share(id);
                println!("Public share: {}",HexFmt(pk_share.to_bytes().to_vec()));
                Actor::new(id, sk_share, pk_share,sk,pk)
            })
            .collect();
...
type QHB = SenderQueue<QueueingHoneyBadger<Transaction, PeerIdW, Vec<Transaction>>>;

impl Actor {
    fn new(id: usize, sk_share: SecretKeyShare, pk_share: PublicKeyShare,sk:SecretKey,pk:PublicKey) -> Self {
        Actor {
            id,
            sk_share,
            pk_share,
...
impl DecryptionMeeting {
 ...
 println!("pk_set: {}",HexFmt(bincode::serialize(&self.pk_set).expect("").to_vec()));
...
fn main() {
    ...
    let mut society = SecretSociety::new(4, 1);
    let pk = society.publish_public_key();
    println!("Pubkey {}",HexFmt(pk.to_bytes().to_vec()));
    ...
    assert!(res.is_ok());
    assert_eq!(msg, res.unwrap().as_slice());
    let vc:Vec<u8>=hex::decode("0100000000000000000000000000000000000000000000000100000022000000000000001220b766a7f774ad3c903ed5139f016e742ac49660f86f3e30be3e241d8ea7a4426d914c55d6d4a92b21d3762d97b0a3fcaa1932a0050aa46d685db271961e566563b8dee8fedca76f31e82c3a0d40acbeae").unwrap();
    let msg=bincode::deserialize::<<QHB as ConsensusProtocol>::Message>(&vc).unwrap();
    println!("{:?}",&msg);
    println!("{}",HexFmt(bincode::serialize(&msg).unwrap().to_vec()));
}
