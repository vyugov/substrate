...
use primitives::{
	...
	crypto::KeyTypeId, offchain,hbbft_thresh
};
...
impl_wasm_host_interface! {
	impl SubstrateExternals where context {
		...
		ext_hb_node_generate(
			...
			let pubkey = runtime_io::crypto::hb_node_generate(key_type, match (seed) {
				   Some(k) => Some(k.as_bytes().to_vec()),
				   None => None
			}
				   );
		...
		ext_hb_node_sign(
			...
			let signature = runtime_io::crypto::hb_node_sign(key_type, &pub_key, &msg);
		...
		ext_hb_node_public_keys(id_data: Pointer<u8>, result_len: Pointer<u32>) -> Pointer<u8> {
			...
			let keys = runtime_io::crypto::hb_node_public_keys(key_type).encode();
