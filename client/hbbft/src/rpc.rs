use badger_primitives::{AccountBinding, AuthorityPair, SignedAccountBinding};
...
pub trait BadgerRpcApi<AccountId>
{
  ...
  fn bind_account(&self, account: AccountId) -> Result<Bytes>;
}
...
impl<C, Block, AccountId> BadgerRpcApi<AccountId> for BadgerRpcCaller<C, Block>
where
  Block: BlockT,
  ...
  //Balance: Codec,
{
  fn bind_account(&self, account: AccountId) -> Result<Bytes>
  {
    ...
    let pair: AuthorityPair = match self.keystore.read().key_pair_by_type::<AuthorityPair>(
      &persistent_data.authority_set.inner.read().self_id,
    ...
    let sgn = SignedAccountBinding { data: bind, sig: sig };
    Ok(sgn.encode().into())
  }
}
